...
SYNAD
 WAS
 THE***
 AN INPUT/OUTPUT ERROR CONDITION WAS ENCOUNTERED DURING
 EXECUTION OF A CHECK/GET/PUT MACRO INSTRUCTION. EITHER NO
 ROUTINE WAS PROVIDED TO HANDLE THE ERROR, OR DCB=EROPT=ABE
 SPECIFIED OR DEFAULTED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S001 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S001-RC)**--POSSIBLE CAUSE--
 ...
 (SEE SB37)
 COBOL - INCORRECT USE OF BLOCK CONTAINS CLAUSE- HAD RECORDING MODE F WHEN GENERATED, BUT NOT WHEN READ
 ...
 S002 -      
RECORD IS GREATER THAN 32,768 BYTES, EXCEEDS MAXIMUM TRACK
 LENGTH OR STATED BLOCKSIZE, COULD NOT BE CONTAINED IN ONE
 EXTENT, OR TOO MANY TRACKS SPECIFIED FOR CYLINDER OVERFLOW.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S002 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S002-RC)**--POSSIBLE CAUSE--
 DCB HAD THE WRONG BLOCK SIZE
EXISTS
 IEHMOVE - ATTEMPT TO RENAME DATA SET WHEN NEW NAME ALREADY
 ...
 OPERATION.  THE RECORD OR BLOCK LENGTH PLUS THE KEY LENGTH
 AND
 ...
 VARIABLE LENGTH RECORD SPECIFIES A LENGTH GREATER THAN
 32,752.
 ...
 THE DATASET USES THE VARIABLE RECORD FORMAT. THE LENGTH
 VALUE OF
 THE RDW IS EITHER: LESS THAN 4, GREATER THAN 32,767, GREATER
 THAN
 THE BLOCKSIZE SPECIFIED IN THE DCB, OR LESS THAN 5 IF ASA
 CONTROL
 ...
 CAPACITY, AND THE RECORD FORMAT DOES NOT USE THE TRACK
 OVERFLOW
 FEATURE.
 S002 - 20 - THE ERROR OCCURRED DURING THE CREATION OF A DIRECT DATASET.
 A WRITE MACRO WAS ISSUED CAUSING A SECONDARY EXTENT TO BE
 OBTAINED.
 THE BLOCK WILL NOT FIT ON THE AMOUNT OF SPACE ALLOCATED FOR
 THE
 SECONDARY EXTENT.
 S002 - 24 - THE ERROR OCCURRED DURING THE CREATION OF A DIRECT DATASET.
 A WRITE MACRO WAS ISSUED TO WRITE OUT A BLOCK LARGER THAN
 THE
 PRIMARY EXTENT ON THE PREALLOCATED DATASET. THIS COULD ALSO
 OCCUR
 ANY
 IF ALLOCATION OF THE PRIMARY EXTENT WAS NON-CONTIGUOUS AND
 OF THE SECONDARY EXTENTS WERE SMALLER THAN THE BLOCK.
 S002 - 28 - THE ERROR OCCURRED DURING THE CREATION OF A DIRECT DATASET.
 DURING THE EXECUTION OF OPEN IT WAS DETECTED THAT THE
 BLOCKSIZE
 ...
 S002 - 30 - ONE OF THE FOLLOWING IS TRUE:- A READ WAS ISSUED BUT THE LENGTH TO BE READ WAS ZERO.- A QSAM PUT WITH MOVE MODE WAS ISSUED FOR RECFM=F, FB OR U
...
ALREADY HAD THE MAXIMUM NUMBER OF MEMBERS (2**19 - 2).
 ...
 BECAUSE THE VALUE OF NCP INT THE DCB HAD ALREADY BEEN
 REACHED.
 ...
 USED FOR PDSE PROCESSING WITH INFORMATION CONCERNING THE
 MEMBER
 BEING ACCESSED.
 ...
 S002 - A0 - INCORRECT ATTEMPT TO ACCESS A PDSE DIRECTORY. EITHER:- WRITE ATTEMPTED TO THE DIRECTORY OF A PDSE.- READ ATTEMPTED TO THE DIRECTORY OF A PDSE AND EITHER THE
 RECFM WAS NOT F WITH BLKSIZE >= 256
 ==OR==
 RECFM WAS NOT U WITH BLKSIZE >= 256.
 S002 - A4 - UNABLE TO COMPLETE THE READ AGAINST THE PDSE DIRECTORY
 DUE TO AN ERROR RETURN CODE FROM AN SMS SERVICE USED TO
 OBTAIN
 ...
 EITHER:- RDW LENGTH IS LESS THAN 4.- LENGTH OF SPANNED RECORD IS GREATER THAN 32760.- THE SUM OF THE LENGTHS IN THE RDWS PLUS 4 DOES NOT EQUAL
 THE
 BUFFER.
 LENGTH SPECIFIED IN THE BDW.- RECFM DOES NOT SPECIFY BLOCKED BUT MORE THAN 1 RDW IN
 S002 - B0 - EITHER RECFM=F AND DCBBLKSI DOES NOT EQUAL DCBLRECL,
 OR RECFM=FB AND DCBBLKSI IS NOT A MULTIPLE OF DCBLRECL.
THE ERROR OCCURED DURING END-OF-BLOCK PROCESSING USING
 S003 -      
EITHER
 EXECUTED
 THE***
 BSAM OR QSAM.  THE ERROR OCURRED BECAUSE THE I/O MACRO
 INSTRUCTIONS FOR A 3525-ASSOCIATED DATA SET WERE NOT
 IN THE PROPER SEQUENCE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S003 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S003-RC)**
 ...
 S004 -      
THE ERROR OCCURED DURING OPEN PROCESSING USING EITHER BSAM
 OR
 THE***
 QSAM BECAUSE A CONFLICTING OR INVALID DCB PARAMETER (FUNC OR
 RELATED PARAMETER) IS SPECIFIED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S004 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S004-RC)**
 ...
 S005 -      
THE ERROR OCCURRED BECAUSE OF AN INVALID DECB DURING READ
 PROCESSING USING BSAM.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S005 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S005-RC)**
 ...
 S006 -      
THE PROGRAM HAD AN INCORRECT AUTHORIZATION TO RUN A 1419.
 S008 -      
CHECK
 S013-      
THE***
 THE ERROR OCCURRED DURING EXECUTION OF A SYNAD ROUTINE.  THE
 ROUTINE WAS GIVEN CONTROL FOLLOWING EXECUTION OF A BSAM
 MACRO INSTRUCTION.
 CONFLICTING OR UNSUPPORTED PARAMETERS IN DCB; MEMBER NAME
 SPECIFIED IN DD NOT FOUND; NO DIRECTORY ALLOCATION
 SUBPARAMETER IN DD.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S013 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S013-RC)**--POSSIBLE CAUSE--
 DCB PARAMETERS CONFLICT, NO BLKSIZE OR NOT VALID MULTIPLE OF
 LRECL
 ...
 JCL - TRIED TO CREATE A PDS WITHOUT ALLOCATING DIRECTORY
 BLOCKS
 S013- 04 - AMERICAN NATIONAL STANDARD LABELS WERE SPECIFIED IN THE
BY
 LABEL PARAMETER OF THE DD STATEMENT, BUT ARE NOT SUPPORTED
 THE SYSTEM. CORRECT THE LABEL PARAMETER.- OPTCD=Q WAS SPECIFIED BUT IS NOT SUPPORTED BY THE SYSTEM.
 ...
 SPECIFIED A 7-TRACK TAPE DRIVE.  ANSI LABELS A VALID ONLY
 FOR
 9-TRACK UNITS.
 ...
 DCB SPECIFIES DSORG=PO, BUT THE DSCB INDICATES THE DATASET
 IS
 NOT PARTITIONED. CHANGE DSORG TO PS, OR CREATE DATASET AS
 PDS.- AN OPEN MACRO WAS ISSUED WITH INPUT SPECIFIED.  THE DSCB
 INDICATES THAT DSORG=PO, BUT THE DCB SPECIFIES NEITHER
 DSORG=PO
 NOR PS.
 ...
 THE QUEUED ACCESS TECHNIQUE WITH RECFM=FB, BUT BLKSIZE IS
 NOT
 ...
 SPECIFIED, BUT THE DCB MACRF DID NOT SPECIFY EXCP, GET, OR
 READ.
 ...
 EXCHANGE BUFFERING WAS OPENED FOR INPUT, BUT THE BUFFER
 CONTROL
 BLOCK ADDRESS WAS 0.
 S013- 30 - A SEQUENTIAL DATASET USING THE QUEUED ACCESS TECHNIQUE WITH
 EXCHANGE BUFFERING WAS OPENED FOR OUTPUT, BUT THE BUFFER
 CONTROL
 BLOCK ADDRESS (DCBBUFCB) WAS 0.
 S013- 34 - ONE OF THE FOLLOWING WAS DETECTED:- AN OPEN MACRO WAS ISSUED FOR A DATASET WITH BLKSIZE AND
 BUFL EQUAL TO 0.  THE SYSTEM DETERMINED THAT IT HAD TO
 OBTAIN
 BUFFERS BUT WAS UNABLE TO DO SO.- RECFM WAS SPECIFIED WITH BOTH A AND M ATTRIBUTES.- QSAM, LRECL=0 AND RECFM WAS NOT V OR VB.- RECFM=V OR VB AND LRECL > (BLKSIZE-4).- QSAM AND BLKSIZE=0.- RECFM NOT VARIABLE OR UNDEFINED AND LRECL > BLKSIZE.- BFTEK=A,LRECL=0,RECFM=VBS.
...
S013- 3C - A SEQUENTIAL DATASET WAS OPENED FOR INPUT OR OUTPUT,
 BUT THE BUFFER CONTROL BLOCK ADDRESS WAS 0.  THIS TYPE OF
 ERROR
 ...
 DEVICE FOR WHICH CHAINED SCHEDULING WAS SPECIFIED, BUT THE
 BUFFER
 CONTROL BLOCK ADDRESS WAS 0.
 S013- 48 - AN OPEN MACRO WAS ISSUED FOR A SEQUENTIAL DATASET USING THE
 QUEUED ACCESS TECHNIQUE, BUT THE BUFFER CONTROL BLOCK
 ADDRESS WAS
 ZERO.
 S013- 4C - AN OPEN MACRO WAS ISSUED FOR A SEQUENTIAL DATASET USING THE
 QUEUED ACCESS TECHNIQUE.  THE SYSTEM DETERMINED THAT A
 BUFFER
 POOL EXISTED FOR THIS DATASET AND MADE THE APPROPIATE TEST
 SHOWN
 BELOW WITH UNSATISFACTORY RESULTS:- IF THE DATA WAS TO BE SENT DIRECTLY TO A UNIT RECORD
 DEVICE (NO
 SPOOLING), THE BUFFER LENGTH VALUE IN THE BUFFER CONTROL
 BLOCK
 HAD TO BE EQUAL TO OR GREATER THAN THE VALUE SPECIFIED IN
 THE
 DCB FOR LOGICAL RECORD LENGTH (LRECL).- OTHERWISE, THE BUFFER LENGTH VALUE IN THE BUFFER CONTROL
 BLOCK
 HAD TO BE EQUAL TO OR GREATER THAN THE VALUE SPECIFIED IN
 THE
 DCB FOR BLKSIZE.
 ...
 QUEUED ACCESS TECHNIQUE.  THE DATASET CONTAINED SPANNED
 VARIABLE
 LENGTH RECORDS LARGER THAN 32,756 BUT GET LOCATE MODE WAS
 NOT USED.
 ALTERNATIVELY, BFTEK=A WAS SPECIFIED WITH MACRF=GM OR
 MACRF=PM.
 S013- 60 - AN OPEN MACRO WAS ISSUED FOR A DATASET WITH A DCB
 SPECIFYING RECFM=F, AND BLKSIZE WAS NOT EQUAL TO LRECL.
 CORRECT
 ...
 A CONFLICT EXISTS AMONG LABEL PARAMETERS ON THE DD
 STATEMENT,
 AND DCBRECFM, DCBOPTCD, DCBBUFOF, AND DCBUSASI GIVE THE
 APPEARANCE
OPTCD=Q
 OF MIXED USASI AND EBCDIC ATTRIBUTES TO THE DATASET; OR
 WAS SPECIFIED FOR A DATASET ON A DEVICE OTHER THAN MAGNETIC
 TAPE.
 ...
 DATASET. THE BUFL PARAMETER IN THE DCB WAS SPECIFIED AS
 ZERO.
 ...
 CONTROL BLOCK IS NOT THE SAME AS THAT SPECIFIED IN THE
 DCBBUFNO.
 ...
 ENTRY SUBSYSTEM.  THE FAILING DCB IS NOT OPENED, HOWEVER THE
 TASK
 IS NOT TERMINATED. PROCESSING CONTINUES FOR OTHER DCBS
 OPENED.
 S013- C4 - DURING THE CREATION OF A DIRECT DATASET, THE OPEN ROUTINE
 FOUND THAT THE DCB SPECIFIED READ(R) OR GET(G) IN THE MACRF
 FIELD.
 ONLY WRITE LOAD (WL) IS ALLOWED.
 S013- C8 - THE OPEN SUBSYSTEM EXECUTOR MODULE WAS PASSED AN ERROR
 RETURN CODE IN REGISTER 15 AFTER ISSUING THE IEFSSREQ MACRO
 TO
...
ASSOCIATED
 BUT THE LOGICAL RECORD INTERFACE IS NOT PRESENT IN THE
 ...
 S016 -      
AN ERROR OCCURRED IN SAM SIO/PAGE FIX OR DASD END-OF-BLOCK
 MODULES BECAUSE OF ONE OF THE FOLLOWING:- THE DASD END-OF-BLOCK MODULES ISSUED AN EXCPVR SVC REQUEST
 ON
 GOING TO
 PROTECTED,
 S020 -      
...
DATA
 ...
 HAD NOT COMPLETED.  ONE OF THE FOLLOWINF OCCURRED:- A DATA EXTENT CONTROL BLOCK (DECB) WAS USED TWICE.- MULTIPLE READS OR WRITES WERE ISSUED, BUT ONLY THE LAST
 READ OR WRITE WAS CHECKED.- AN EXCPVR SVC REQUEST ON BEHALF OF A USER REQUEST WAS
 BE ISSUED TO READ A DATA SET THAT IS EXECUTE ACCESS
 BUT THE USER IS NOT IN SUPERVISOR STATE.
 (THE SAM SIO APPENDAGE ISSUED THIS ABEND.)
 DURING AN OPEN FOR A BDAM DATA SET THE CONTROL PROGRAM FOUND
 THAT DCBMACRF DID NOT CONTAIN AN A, K OR I (RC=04), OR THE
 DATA SET CONTAINED ZERO EXTENTS (RC=08).
 A CALLER OF THE ASCBCHAP ROUTINE PASSED TO ASCBCHAP AN ASCB
 ADDRESS THAT HAD BITS SET TO ONE IN ITS HIGH ORDER BYTE.
 THE VPSS ENCOUNTERED A MISSING DEVICE END ON CONTROL PORT 0.
 LNKLST LOOKASIDE (LLA) COULD NOT OBTAIN ALL THE LNKLST
 DIRECTORY ENTRIES.
 AN ABNORMAL TERMINATION OCCURRED IN PRINT SERVICES FACILITY.
 AN ABNORMAL TERMINATION OCCURRED IN PRINT SERVICES FACILITY.
 A PAGING OPERATION HAS NOT COMPLETED SUCCESSFULLY DUE TO ONE
 OF THE FOLLOWING:- A PERMANENT I/O ERROR OCCURRED WHILE ATTEMPTING A PAGE-IN
 SWAP-IN OPERATION. THE DATA BEING PAGED IN OR SWAPPED IN IS- A REAL STORAGE MANAGEMENT ROUTINE OR ANOTHER SYSTEM
 PERFORMING A SERVICE FOR RSM SUFFERED AN INTERMEDIATE ERROR.
 THE FUNCTION PERFORMED IS TERMINATED.- A AUXILIARY STORAGE MANAGEMENT ROUTINE SUFFERED A
 ERROR WHILE USING THE CONTROL REGISTER OF ANOTHER ADDRESS
 TO UPDATE THAT ADDRESS SPACE'S LSQA.
 AN INVALID VIO REQUEST WAS DETECTED BY THE PAGE OPERATIONS
 STARTER ROUTINE OF ASM.  THE SYS1.LOGREC ERROR RECORD
 A HEXADECIMAL REASON CODE.
 JES2 FOUND THAT DATA MANAGEMENT CONTROL BLOCKS OR THE JES2
 UBUF CONTROL BLOCK WAS NO LONGER VALID DURING EXECUTION OF A
S02B -      
S02C -      
S02D -      
S030 -      
...
OR
 ...
 A USER EXIT ROUTINE ISSUED A RETURN CODE GREATER THAN THAT
 SPECIFIED IN THE &MAXRC PARAMETER OF THE $EXIT### MACRO.
 AN ERROR OCCURRED IN JES2 SUPPORT OF AN FSS ADDRESS SPACE.
 THE JES2 MAIN TASK WAS ABENDED BY THE OPERATOR OR A
 CATASTROPHIC ERROR.
 DURING A BISAM OR QISAM OPEN DCBMACRF DID NOT INDICATE A
 VALID MODE OF OPERATION.
 A QISAM I/O ERROR OCCURRED.
 DURING A BISAM OR QISAM OPEN DCBMACRF WAS NOT VALID FOR
 ISAM DATASET TO BE PROCESSED, BUT NOT CREATED OR ITS DCB
 NOT CLOSED AFTER CREATION.--POSSIBLE CUASE--
 DATASET WAS TO BE WRITTEN ON BUT WAS OPENED AS INPUT
 LRECL/BLKSIZE PROBLEM EXISTS - INVALID VALUES OR NOT
 THE ERROR OCCURRED DURING THE EXECUTION OF A QISAM OR BISAM
 BDAM MACRO INSTRUCTION.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S03D OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S03D-RC)**
 S03D - 04 - AN OPEN MACRO WAS ISSUED FOR AN INDEXED SEQUENTIAL OR
 DIRECT DATASET.  THE VOLUME SERIAL NUMBERS ON THE DD
 STATEMENT
 WERE NOT SPECIFIED IN THE SAME ORDER THAT THE DATASET WAS
 CREATED.
...
AN UNAUTHORIZED PROGRAM REQUESTED A RESTRICTED SVC.
 S04B -      
THE***
 AN INVALID OPERATION CODE WAS PASSED TO A TSO LINKAGE ASSIST
 ROUTINE (LAR).
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S04B OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S04B-RC)**
 ...
 S04C -      
THE TERMINAL MONITOR PROGRAM (TMP) WAS INVOKED AT ENTRY
 POINT
 S04E -      
S04F -      
S052 -      
...
IKJEFT1A AND THE TMP DETACHED A PROGRAM THAT COMPLETED WITH
 ...
 INTERNAL IBM DATABASE 2 (DB2) ERROR.
 DB2 ABNORMAL TERMINATION TO AVOID DATA INTEGRITY PROBLEMS.
 A PROGRAM MADE AN INVALID PC/AUTH REQUEST. CHECK THE CODE
 REGISTER 15.
 A PROGRAM MADE A PC/AUTH REQUEST BUT AN ERROR PREVENTED
 NORMAL COMPLETION. CHECK THE CODE IN REGISTER 15.
 S064 -      
S071 -      
SYSTEM'S
 AN ADDRESS SPACE THAT HOLDS A CROSS MEMORY LOCAL LOCK (CML
 LOCK) TERMINATED ABNORMALLY BEFORE IT FREED THE LOCK.
 THE OPERATOR PRESSED THE RESTART KEY TO ACTIVATE THE
 RECOVERY AND TERMINATION PROCESS.  THE PROGRAM RUNNING AT
 THE
THROUGH
 TIME THE OPERATOR PRESSED THE RESTART BUTTON WAS SENT
 S078 -      
ABEND PROCESSING BECAUSE THE OPERATOR DETERMINED IT WAS IN A
 NONCANCELABLE LOOP OR WAIT STATE.
 RCT (REGION CONTROL TASK) PROCESSING HAS RECEIVED AN
 UNACCEPTABLE RETURN CODE FROM AN INVOKED ROUTINE AND CANNOT
 CONTINUE PROCESSING.  RCT RECOVERY RECORDS THE ERROR IN
 SYS1.LOGREC, PERFORMS RESOURCE CLEAN-UP, REQUESTS A
 SYS1.DUMP OF
 THE ADDRESS SPACE AND THEN REQUESTS ADDRESS TERMINATION.
 S079 -      
RCT (REGION CONTROL TASK) ATTENTION EXIT PROCESSING HAS
 RECEIVED AN UNACCEPTABLE RETURN CODE FROM AN INVOKED ROUTINE
 AND
 IN
 CANNOT CONTINUE PROCESSING.  RCT RECOVERY RECORDS THE ERROR
 SYS1.LOGREC, PERFORMS RESOURCE CLEAN-UP, REQUESTS A
 SYS1.DUMP OF
 S07C -      
CURRENT
...
S0A9 -      
THE ADDRESS SPACE AND THEN REQUESTS ADDRESS TERMINATION.
 SUPERVISOR CONTROL RECOVERY HAS DETECTED AN ERROR WHICH
 REQUIRES THAT THE CURRENT TASK BE TERMINATED OR THAT THE
 ADDRESS SPACE BE TERMINATED.
 DURING RECOVERY, VERIFICATION OF THE TCB QUEUE ASSOCIATED
 THE TERMINATED ADDRESS SPACE RESULTED IN AN EMPTY TCB QUEUE.
 THE ADDRESS SPACE OR JOB IS TERMINATED. RESUBMIT THE JOB.
 DURING Z NET,CANCEL COMMAND PROCESSING, VTAM FOUND THAT A
 HAD NOT SPECIFIED A TPEND EXIT ROUTINE, OR VTAM COULD NOT
 A USER'S TPEXIT ROUTINE.
 VTAM DETECTED AN ERROR DURING EXECUTION OF AN APPLICATION
 ...
 EITHER A HALT NET,CANCEL COMMAND WAS ENTERED OR AN ERROR
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S0B0 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S0B0-RC)**
 ...
 ATTEMPTING TO READ OR WRITE A RECORD, WHICH IS NOT 176
 BYTES, IN
 MOVE MODE).
 ...
 FIRST TIME AND HAS EITHER PASSED A NON-EXISTING ID OR HAS
 FAILED
 TO PASS ONE AND THE BLOCK DOES NOT HAVE AN IMBEDDED ID).
 S0B0 - 20 - INVALID BLOCK POINTER (WRITE LOCATE IS ATTEMPTING TO WRITE
 AND IS PASSING A BLOCK POINTER WHICH IS NOT VALID FOR THE
 SVA
 REQUESTED).
 S0C1 -      
OPERATION EXCEPTION - AN OPERATION CODE IS NOT ON THE
 MACHINE
 SUBROUTINE
 AND A
 PROCEDURE
...
ERROR--POSSIBLE CAUSE--
 ...
 BAD LOAD MODULE, POSSIBLE BAD OBJECT DECK OR MISSING
 FORTRAN - MISSING DIMENSION STATEMENT, SAME NAME FOR ARRAY
 SUBROUTINE
 COBOL - SUBROUTINE PROG ID WAS THE SAME AS THE ENTRY NAME
 COBOL - TRIED TO CALL WITHIN COBOL F SORT INPUT/OUTPUT
 COBOL - TRIED TO CALL A SUBROUTINE WHICH COULD NOT BE FOUND
 COBOL - INCOMPLETE DCB FOR SORTIN
 COBOL - USING SORT VERB, DDNAME WAS NOT SORTOUT WHEN THE
 OPTION WAS USED.
 COBOL - EXECUTING SORT-USING AFTER OPENING SORTIN
 PRIVILEGED OPERATION EXCEPTION--POSSIBLE CAUSE--
 UNINTENTIONAL BRANCH TO INVALID INSTRUCTION DUE TO SUBSCRIPT
NAMES
 COBOL - MISSING PERIOD AT END OF PARAGRAPH OR PARAGRAPH- MISSING GOBACK AFTER SORT VERB - LOGIC FELL INTO INPUT
 PROCEDURE
 S0C3 -      
S0C4 -      
PARMS
...
V=R
 S0C6 -      
SPECIFIED.- ACCEPT VERB EXECUTED WHEN NO SYSIN DD WAS AVAILABLE.
 EXECUTION EXCEPTION - THE SUBJECT INSTRUCTION OF EXECUTE
 IS ANOTHER EXECUTE.--POSSIBLE CAUSE--
 UNINTENTIONAL BRANCH, LOSS OF ADDRESSABILITY.
 PROTECTION EXCEPTION - A VIRTUAL ADDRESS COULD NOT BE
 TRANSLATED INTO A REAL ADDRESS.--POSSIBLE CAUSE--
 INVALID ADDRESS WAS REFERENCED DUE TO SUBSCRIPT ERROR OR BAD
 ...
 THE PAGE WAS PAGED OUT AND THE ROUTINE REQUESTING THE CODE
 DISABLED FOR I/O INTERRUPTS (INTERRUPT CODE 11)
 COBOL - IN GROUP MOVE, RECEIVING RECORD VARIABLE LENGTH
 WRONG- TRIED MOVING VAR. LENGTH REC. LARGER THAN TARGET FIELD MAX- TRIED TO READ OR WRITE A FILE WHICH WAS NOT OPEN- USED DD DUMMY AND LOGIC SAYS AT END MOVE HIGH VALUES TO FD- TRIED TO CALL WITHIN COBOL F SORT INPUT/OUTPUT PROCEDURE- U WITH SORT - TRIED TO GOBACK IN THE OUTPUT PROCEDURE
 ADDRESSING EXCEPTION - AN ADDRESS BEYOND THE AVAILABLE REAL
 STORAGE WAS REQUESTED--POSSIBLE CAUSE--
 SEE S0C1 FOR MORE INFORMATION
 SUBSCRIPT ERROR - REFERENCED BEYOND TABLE IN PROGRAM RUNNING
 COBOL - PERFORMED PROCEDURE NOT EXITED PROPERLY- RECORD DESCRIBED TOO SHORT- REFERENCED A RECORD WITHOUT GIVING A READ- TRIED TO USE WRITE WITHOUT FROM OPTION ON APPLY WRITE
 ONLY FILE
 SPECIFICATION EXCEPTION - AN INCORRECT BOUNDARY WAS
 USUALLY CAUSED BY UNINTENTIONAL BRANCH TO INVALID
 INSTRUCTION.
 S0C7 -      
S0C8 -      
S0C9 -      
DATA EXCEPTION - DECIMAL DATA IS INCORRECT OR IMPROPERLY
 OVERLAPPED OR NOT VALIDLY INITIALIZED.--POSSIBLE CAUSE--
 SUBSCRIPT ERROR, REFERENCED BEYOND TABLE
 COBOL - WORKING STORAGE NOT INITIALIZED- BAD DATA, SHOULD CHECK DATA FOR ERRORS- GARBAGE IN A FIELD BEING TESTED OR DISPLAYED- MOVE ZEROES TO GROUP LEVEL IS DISPLAY, HAD SUBLEVELS
 THAT WERE NOT- PERIOD MISSING AFTER IMPERATIVE STATEMENTS WITHIN AT END
 CLAUSE- BINARY FIELD IN AN ARITHMETIC OPERATION IS NOT LARGE
...
THE***
 ...
 NOTE - FORTRAN WILL SET THE RESULT TO ZERO AND CONTINUE
 SIGNIFICANCE EXCEPTION
 A FLOATING POINT ADDITION OR SUBTRACTION RESULTS IN AN ALL
 FRACTION
 NOTE - THIS IS MASKED IN FORTRAN
 FLOATING POINT DIVIDE EXCEPTION - DIVIDE BY ZERO
 NOTE - THIS ERROR IS DETECTED AND NOTED BY FORTRAN
 A PROGRAM CHECK WAS DETECTED; AN INTERRUPTION CODE OF 18
 X'12' (TRANSLATION SPECIFICATION EXCEPTION) HAS OCCURRED.
 IS INVALID DATA IN EITHER CONTROL REGISTERS ZERO OR ONE, OR
 SEGMENT OR PAGE TABLE.  THE ERROR IS THE RESULT OF A
 ERROR OR A PROGRAM RUNNING IN KEY ZERO HAS CAUSED DATA
 A PROGRAM CHECK, INTERRUPTION CODE 19 X'13' (SPECIAL
 ERROR) HAS OCCURRED.  A PRIVILEGED PROGRAM ISSUED A SET
 MASK. THIS INSTRUCTION IS NOT VALID IN OS/VS2 (SINCE RELEASE
 A PROGRAM CHECK, INTERRUPTION CODE 38 (PAGE FAULT ASSIST
 ERROR) HAS OCCURRED.  EITHER A HARDWARE ERROR OCCURRED OR A
 PROGRAM RUNNING IN KEY ZERO CAUSED DATA DAMAGE.
 AN INVALID OR UNRECOGNIZED PROGRAM INTERRRUPT OCCURRED.--POSSIBLE CAUSE--
 THIS LEVEL OF MVS IS NOT THE CORRECT ONE FOR THE HARDWARE,
 ESPECIALLY IF THE INTERRUPT CODE IS UNKNOWN.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S0E0 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S0E0-RC)**
 ...
 REFERS TO IS NOT EQUAL TO THE NUMBER IN THE ACCESS LIST
 ENTRY.
...
HAS NO STACK ENTRIES.
 ...
 THAT HAS THE UNSTACK SUPPRESSION BIT SET. MVS SETS THIS BIT
 IN
 A LINKAGE STACK ENTRY TO PREVENT SVC ROUTINES FROM
 UNSTACKING
 S0E1 -      
S0E2 -      
FREE, OR
...
THE***
 ...
 THE ERROR OCCURRED WHILE PROCESSING THE FIRST EXCP OF A VIO
 DATASET. PLEASE REPORT THIS PROBLEM TO THE SYSTEMS GROUP.
 THE ERROR OCCURRED WHILE PROCESSING THE FIRST EXCP OF A VIO
 DATASET.  THE SYSTEM WAS UNABLE TO ASSIGN, LOCATE, FIX,
 ACCESS VIO PAGES FOR THE DATASET.  PLEASE REPORT THIS
 TO THE SYSTEMS GROUP.
 THE ERROR OCCURRED WHILE PROCESSING A VIO DATASET.  THE
 WAS UNABLE TO ASSIGN, LOCATE, FIX, FREE, OR ACCESS VIO PAGES
 THE DATASET. PLEASE REPORT THIS PROBLEM TO THE SYSTEMS
 THE ERROR OCCURRED WHILE TRYING TO SCRATCH A VIO DATASET.
 PLEASE REPORT THIS PROBLEM TO THE SYSTEMS GROUP.
 UNABLE TO DISCONNECT THE VIRTUAL TRACK BUFFER FORM THE VIO
 DATASET. PLEASE REPORT THIS PROBLEM TO THE SYSTEMS GROUP.
 UNABLE TO PROCESS A DATA SET THAT HAS ALREADY BEEN
 AN ERROR OCCURRED WHILE PROCESSING A MACHINE CHECK
 INTERRUPTION.  IN AN ATTEMPT TO RECOVER THE SYSTEM THE
 TASK WAS TERMINATED.
 PROGRAM INTERRUPTION IN I/O INTERRUPTION HANDLER
 RECORD DESCRIBED AS WRONG LENGTH
 I/O INTERRUPT PROCESSING ERROR. I/O RECOVERY ROUTINE FAILED.
 CLOBBERED IOB OR OTHER SVC PARAMETERS- SEE S0C1
 MACHINE CHECK INTERRUPTION
 DISK I/O FAILURE OR MACHINE TROUBLE
 THE ISSUER OF AN SVC WAS EITHER IN SRB MODE, HELD A LOCK,
 WAS DISABLED, OR WAS IN CROSS MEMORY MODE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S0F8 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S0F8-RC)**
 ...
 S0F9 -      
THE ERROR OCCURRED WHILE TRYING TO ACQUIRE OR RELEASE AN
 SVRB.
 THE***
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S0F9 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S0F9-RC)**
S0F9 - 00 - LSQA HAS BEEN EXHAUSTED AND AN SVRB CANNOT BE CREATED TO
 ...
 S0FB -      
A TRANSLATION EXCEPTION HAS OCCURRED. INVALID CONTENTS
...
CONTROL
 PROTECTION
 THE***
 A TRANSLATION ERROR OCCURRED WHILE REFERENCING DATA NOT IN
 THE HOME ADDRESS SPACE. BAD DAT TABLES OR WRONG STO ADDRESS.
 A TYPE 6 SVC REQUESTED THE SVC FIRST LEVEL INTERRUPT HANDLER
 (FLIH) TO SCHEDULE A CROSS MEMORY SRB.
 T6EXIT MACRO POINTS TO SRB WHERE SRBASCB IS NOT HOME ASCB.
 AN ERROR WAS DETECTED WHILE RMF WAS SAMPLING DATA ABOUT
 THE STATE OF THE SYSTEM.
 A DEVICE TO BE USED WAS NOT OPERATIONAL OR A PSEUDO DEVICE
 PREVIOUSLY ALLOCATED HAS BEEN REALLOCATED.
 DURING EXECUTION OF A WAIT MACRO, THE PROBLEM PROGRAM
 SPECIFIED MORE EVENTS THAN THERE WERE ECBS.
 THE ERROR OCCURRED DURING EXECUTION OF A POST MACRO
 INSTRUCTION.  THE CONTROL PROGRAM FOUND AN INVALID EVENT
 BLOCK ADDRESS, OR THE ECB WAS IN A STORAGE AREA WITH A
 KEY DIFFERENT FROM THAT OF THE TASK ISSUING THE MACRO.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S102 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S102-RC)**
 ...
 S104 -      
GETMAIN ERROR.  AN AUTHORIZED PROGRAM REQUESTED VIRTUAL
 STORAGE THAT REQUIRES MORE REAL STORAGE THAN IS AVAILABLE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S104 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S104-RC)**
 ...
 S106 -      
AN ERROR WAS DETECTED BY THE CONTROL PROGRAM WHEN IT
 ATTEMPTED
 THE***
 TO FETCH THE REQUESTED PROGRAM INTO VIRTUAL STORAGE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S106 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S106-RC)**--POSSIBLE CAUSE--
 PREVIOUS LKED STEP FAILED TO OUTPUT A SYSLMOD, NEEDS LARGER
 SIZE=
...
S106 - 0D - THE CONTROL PROGRAM FOUND AN INVALID RECORD TYPE IN THE
 ...
 MODULE HAD AN ERROR WHICH CAUSED THE CHANNEL PROGRAM TO
 FAIL,
 OR A LINKLIST LIBRARY EXPANDED INTO ANOTHER EXTENT SINCE
 IPL.
 CHECK REGISTER 0:
 ...
 LOAD MODULE.
 S10A -      
GETMAIN ERROR.  A PROGRAM REQUESTED VIRTUAL STORAGE THAT
 REQUIRES MORE REAL STORAGE THAN IS AVAILABLE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S10A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S10A-RC)**
 ...
 S10B -      
TIME SERVICE ROUTINE ERROR - ERROR IN INPUT PARAMETERS.
 S10D -      
S10E -      
S112 -      
MACRO.
 THE***
 JOBSTEP TASK ABENDED WHEN A SUBTASK THAT IS ALSO A JOBSTEP
 TASK IS IN STEP-MUST-COMPLETE STATUS.
 SPIE MACRO ERROR. THE PICA ADDRESS IS INVALID.
 INVALID INPUT WAS PASSED TO SVC 18 BY THE BLDL OR FIND
 REQUIRES MORE REAL STORAGE THAN IS AVAILABLE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S112 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S112-RC)**--POSSIBLE CAUSE--
 ...
 S113 -      
AN ERROR OCCURRED DURING AN OPEN WITH A TYPE=J OPERAND.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S113 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S113-RC)**
...
MULTI-VOLUME VSAM FILES CANNOT BE OPENED WITH A DCB.
 ...
 SET OPENED WITH A DCB.  ONLY VALID OPTIONS ARE INPUT AND
 UPDAT.
 S113 - 38 - AN OPEN WAS ISSUED FOR A VSAM DATA SET WITH A DCB,
 BUT THE VTOC DOES NOT INDICATE THAT IT IS A VSAM DATA SET.
 S115 -      
SVC 21 (STOW) ERROR.  THE DCB DID NOT POINT TO A VALID DEB,
 OR THE DEB DID NOT POINT BACK TO THE DCB.
 S117 -      
S118 -      
THE***
 AN I/O ERROR OCCURRED DURING A BSAM CLOSE TYPE=T MACRO.
 INVALID PARAMETERS WERE PASSED TO DEVTYPE (SVC 24).
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S118 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S118-RC)**
 ...
 S11A -      
A PROTECTION CHECK OCCURRED WHEN THE SYSTEM TRIED TO STORE
...
TAPE.
 THE***
 A MODULE ISSUED SVC 33 (IOHALT) WITH THE LOW ORDER BIT OF
 GPR1 OFF, WHICH INDICATES THE PRE-XA (2-BYTE UCB ADDR)
 EITHER THE OPERATOR OR THE JOB ENTRY SUBSYSTEM CANCELLED THE
 JOB, REQUESTING A DUMP. CHECK THE JOB LOG LISTING FOR MORE
 AN INVALID ADDRESS LIST WAS SPECIFIED FOR AN EXTRACT MACRO.
 LIST NOT ON A FULLWORD BOUNDARY OR NOT IN JOB STEP'S
 INVALID SEGMENT TABLE IN AN OVERLAY PROGRAM
 SEE S0C1
 ERROR FOUND IN INPUT PARAMETERS TO TTIMER SERVICE ROUTINE.
 THE DEQ MACRO SPECIFIED A RESOURCE NOT PREVIOUSLY SPECIFIED
 BY AN ENQ MACRO.  THAT IS, THE PROGRAM HAD NOT REQUESTED
 OF A RESOURCE IT WAS ATTEMPTING TO RELEASE.
 I/O ERROR IN END OF VOLUME LABEL PROCESSING ON MAGNETIC
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S137 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S137-RC)**
 ...
 DATASET OPENED WITH THE INPUT OR INOUT OPTION, OR READING
 THE
 ...
 OPENED FOR INPUT OR INOUT, OR READING THE TRAILER LABEL FOR
 A
DATASET OPENED FOR RDBACK.
 S137 - 18- AN I/O ERROR OCCURRED WHILE POSITIONING A MAGNETIC TAPE
 DATASET AT THE FIRST DATA RECORD, OR ON A READ BLOCK ID
 COMMAND
 ...
 REQUESTED TO SPECIFY WHETHER THE VOLUME COULD BE USED IN
 SPITE
 OF THE EXPIRATION DATE, THE OPERATOR DID NOT REPLY 'U'.
 ASK THE OPERATOR TO REPLY 'U' OR SPECIFY ANOTHER VOLUME
 SERIAL.
 ...
 THE LABEL VALIDATION EXIT REQUESTED OPEN/EOV TO REJECT THE
 VOLUME.
...
PASSED BY
 THE***
 THE ERROR OCCURRED DURING THE EXECUTION OF AN ENQ MACRO.
 TWO ENQ MACROS WERE ISSUED FOR THE SAME RESOURCE IN THE SAME
 WITHOUT AN INTERVENING DEQ MACRO.  THE SECOND ENQ MACRO DID
 SPECIFY TEST, USE, OR HAVE IN ITS RET OPERAND.
 THE TASK WHICH CREATED A SUBTASK HAS ISSUED A DETACH FOR
 THAT SUBTASK, SPECIFYING STAE=NO, BEFORE THAT SUBTASK HAS
 TERMINATED.
 THE ROUTINE ATTEMPTED TO EXECUTE THE STATUS MACRO
 FOR OTHER THAN THE STOP, STOP SYNCH, OR START FUNCTION AND
 IN SUPERVISOR KEY (0-7).
 THE CONTROL PROGRAM REQUESTED THAT A DEBCHK FUNCTION BE
 PERFORMED ON A DATA EXTENT BLOCK OBTAINED FROM THE DCB
 THE PROGRAM. THAT FUNCTION COULD NOT BE COMPLETED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S16E OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S16E-RC)**
 ...
 THAT THE DCBDEBAD FIELD OF THE DCB POINTES TO THE DEB, BUT
 THE
 DEBDCBAD FIELD OF THE DEB MUST POINT TO THE DCB WHEN
 TYPE=VERIFY,
 ADD, OR DELETE.
 S16E - 14 - AM VALUE DOES NOT EQUAL DEBAMTYP VALUE. WHEN A DEB POINTER
IF
 IS ADDED TO THE TABLE, THE ACCESS METHOD POINTER (AM) VALUE,
 GIVEN IS PLACED IN THE DEBAMTYP FIELD OF THE DEB.  IF NO AM
 VALUE
 ISSUED
 SAME
 NO
 IS CODED, A 0 IS INSERTED IN THE FIELD. SUBSEQUENT DEBCHKS
 TO VERIFY OR DELETE THAT DEB POINTER MUST EITHER SPECIFY THE
 AM VALUE OR OMIT THE OPERAND.  WHEN THE OPERAND IS OMITTED,
 COMPARISON IS MADE.
 S16E - 18 - DEB NOT ON TCB CHAIN FOR TYPE = ADD. BEFORE THE DEB
 POINTER CAN BE ADDED TO THE TABLE, THE DEB ITSELF MUST BE
 QUEUED
 ON THE CURRENT TCB DEB CHAIN.
 S16E - 1C - DEBAMTYP OR DEBTBLOF = 0 FOR TYPE = ADD. VALUES OTHER THAN
 0 INDICATE A POINETR TO THIS DEB ALREADY EXISTS IN THE DEB
 TABLE.
 S16E - 20 - DEB TABLE CONTAINS 32760 BYTES AND TYPE = ADD. THE CURRENT
 DEB TABLE DOES NOT HAVE SPACE FOR THIS NEW DEB POINTER.  TO
 INCREASE THE TABLE SIZE BY THE REQUIRED INCREMENT OF 24
 WOULD
 CAUSE THE TABLE TO EXCEED ITS MAXIMUM SIZE.
 S171 -      
IF
 THE***
 THE REAL STORAGE MANAGER WAS INVOKED WITH A REQUEST FOR A
 PGFIX, PGFREE, PGLOAD, OR PGOUT SERVICE AND THE REQUEST WAS
 ILLEGAL OR INVALID.  THE REQUEST IS CONSIDERED TO BE ILLEGAL
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S171 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S171-RC)**--POSSIBLE CAUSE--
 INPUT PARAMETER ERROR IN VIRTUAL SUBAREA LIST (VSL):- VSL NOT ON A FULLWORD BOUNDARY.- VSL NOT IN FIXED STORAGE.- UNDEFINED OR CONFLICTING OPTION FLAGS.
 END ADDRESS OF RANGE NOT GREATER THAN BEGINNING ADDRESS.
 AN OPTION WAS SPECIFIED WHICH IS NOT SUPPORTED BY MVS.
 THESE ARE
 RSAOPT AND ECBIND.- VSL IS STORE PROTECTED FROM THE CALLER.
 INPUT PARAMETER ERROR ON ECB:- ECB NOT ON FULLWORD BOUNDARY.- ECB IS STORE-PROTECTED FROM CALLER.- ECB NOT SPECIFIED FOR PGFIX.- ECB SPECIFIED FOR PGOUT.
...
THE***
 ...
 STORAGE THAT REQUIRES MORE REAL STORAGE THAN IS AVAILABLE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
***S178 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S178-RC)**
 ...
 S201 -      
THE WAIT MACRO EXPANSION CONTAINED AN INVALID EVENT CONTROL
 BLOCK (ECB) ADDRESS OR THE PROGRAM IS NOT IN THE SAME KEY AS
 THE ECB.
 S202 -      
THE***
 THE POST MACRO INSTRUCTION ENCOUNTERED AN ERROR WHILE
 PROCESSING AN EVENT CONTROL BLOCK (ECB).
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S202 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S202-RC)**
 S202 - 00 - THE CONTROL PROGRAM FOUND AN INVALID REQUEST BLOCK (RB)
 ADDRESS IN THE 3 LOW-ORDER BYTES OF THE ECB SPECIFIED IN A
 WAIT
 MACRO. THE RB ADDRESS MUST REMAIN IN THE ECB UNTIL A POST IS
 ISSUED.
 ...
 GETMAIN ERROR.  NOT ENOUGH REAL STORAGE AVAILABLE TO BACK A
 MINIMUM
 NUMBER OF VSM CELLS (IN LSQA) DURING LOCAL CELL POOL
 EXPANSION.
 ...
 EXPANSION.
 S206 -      
THE ADDRESS OF THE PARAMETER LIST, OR ONE OF THE PARAMETERS
 ...
 S213 -      
THE***
 DSCB NOT FOUND; I/O ERROR IN READING OR WRITING DSCB.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S213 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S213-RC)**--POSSIBLE CAUSE--
 ...
 FORMAT-1 DSCB COULD NOT BE FOUND ON THE FIRST VOLUME
 SPECIFIED
 ...
 OR INDEXED SEQUENTIAL DATASET, OR THE FORMAT-1 DSCB COULD
 NOT BE
...
S213 - 18 - AN I/O ERROR OCCURRED WRITING BACK A FORMAT-1 DSCB.
 ...
 SPECIFING UNIT=SYSDA, BUT THE UNIT ALREADY CONTAINED 127
 USERS,
 ...
 DATA SET TO UNMOVEABLE WHICH IS NOT ALLOWED ON SMS-MANAGED
 VOLUMES.
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S214 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S214-RC)**
...
THERE IS
 EITHER THE OPERATOR OR THE JOB ENTRY SUBSYSTEM CANCELLED THE
 JOB.  CHECK THE JOB LOG LISTING FOR MORE INFORMATION.  IF
 NO APPARENT EXPLANATION, CONTACT OPERATIONS BEFORE
 RESUBMITTING.--POSSIBLE CAUSE--
 LINE OR CARD ESTIMATE EXCEEDED
 JCL ERROR CAUSED MOUNT REQUEST FOR INVALID OR NON-EXISTING
 VOLUME
 REQUEUED)
 S233 -      
REQUESTED DEVICES NOT CURRENTLY AVAILABLE (JOB MAY BE
 INVALID PARAMETERS HAVE BEEN PASSED TO SVC DUMP.
THE***
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S233 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S233-RC)**
 ...
 IS NOT SERIALIZED (BY SETTING ON THE HIGH ORDER BIT IN
 CVTSDBF)
 ...
 THE ASID WAS LESS THAN ZERO, OR GREATER THAN THE MAXIMUM
 VALUE.
 S233 - 28 - AN ASID SPECIFIED IN THE ASID LIST POINTED TO BY THE
 ASIDLST PARAMETER WAS SYNTACTICALLY INVALID. THE ASID WAS
 LESS
 ...
 S237 -      
THE***
 THE ERROR OCCURRED DURING END-OF-VOLUME LABEL VERIFICATION.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S237 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S237-RC)**--POSSIBLE CAUSE--
 ...
 JFCB ON THE SECOND OR SUBSEQUENT VOLUME OF A MAGNETIC TAPE
 DATASET.
...
REGISTER
 ...
 S240 -      
THE***
 THE ERROR WAS DETECTED DURING EXECUTION OF A DETACH MACRO
 INSTRUCTION.  EITHER 1) THE PARAMETER PASSED TO DETACH IN
 1 WAS NOT A FULLWORD ADDRESS, 2) THE STORAGE KEY OF THAT
 DID NOT MATCH THAT OF THE ISSUER OF THE DETACH, OR 3) THE
 PARAMETER CONTAINED IN THE ADDRESSED FULLWORD OF THE ISSUER
 NOT THE ADDRESS OF A SUBTASK OF THE ISSUER OF THE DETACH.
 AN ERROR OCCURRED DURING EXECUTION OF A RDJFCB MACRO.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S240 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S240-RC)**
...
THAN THE
 ...
 NON
 FIELD
 JOB WAS BEING EXECUTED WHEN SYSTEM FAILURE OCCURRED 
A SYSTEM RESTART WAS PERFORMED
 AN ERROR WAS DETECTED WHEN AN I/O OPERATION WAS REQUESTED;
 *  EITHER THE IOBM FIELD IN THE I/O BLOCK (IOB) IS HIGHER
 MAXIMUM EXTENT COUNT (DEBNMEXT) IN THE DATA EXTENT BLOCK
 *  THE STORAGE PROTECTION KEY OF THE DATA EXTENT BLOCK WAS
 OR THE DEB VALIDITY CHECK ROUTINE RETURNED TO EXCP WITH A
 ZERO RETURN CODE.  IN THE ABNORMAL TERMINATION DUMP, THE TCB
 TCBEXCPD (AT OFFSET C0) POINTS TO THE EXCP PROBLEM
 DETERMINATION
 AREA.  THE ITEMS IN THE PROBLEM DETERMINATION AREA OF
 GREATEST
 DETERMINED
 ELEMENT
 THAN
...
AREA
 ...
 DESCRIBED
 THE***
 INTEREST TO YOU ARE: (ALL OFFSETS IN HEX)- OFFSET 10 CONTAINS A COPY OF THE REGISTERS WHEN EXCP
 THE ERROR CONDITION.- OFFSET 50 CONTAINS THE CONTENTS OF THE REQUEST QUEUE
 (RQE) IF ALLOCATED, WHEN THE PROGRAM CHECK OCCURRED.
 POSSIBLE CAUSES OF A NON-ZERO RETURN CODE FROM IFGDEBCK ARE:- THE DEBTABLE DOES NOT EXIST.- THE DEBTBLOF VALUE IN THE DEB IS LESS THAN ONE OR GREATER
 THE NUMBER OF ENTRIES IN THE DEBTABLE.- THE DEB ADDRESS FROM THE DCB DOES NOT MATCH ANY DEBTABLE
 AN ECB SPECIFIED IN A WAIT MACRO ALREADY HAD ITS WAIT FLAG
 THE ERROR OCCURRED DURING EXECUTION OF A FREEMAIN MACRO:- THE SPECIFIED SUBPOOL COULD NOT BE FOUND.- THE SP PARAMETER WAS SPECIFIED BUT THE VIRTUAL STORAGE
 TO BE RELEASED WAS NOT WITHIN THE SUBPOOL SPECIFIED.- THE SP PARAMETER WAS NOT SPECIFIED BUT THE VIRTUAL STORAGE
 AREA TO BE RELEASED WAS NOT WITHIN SUBPOOL ZERO.- THE SP PARAMETER WAS SPECIFIED CORRECTLY, BUT THE
 OF THE STORAGE AREAS TO BE FREED WERE NOT COMPLETELY
 BY A DESCRIPTOR QUEUE ELEMENT.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
***S305 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S305-RC)**
 ...
 S306 -      
THE ERROR OCCURRED DURING THE EXECUTION OF THE LINK, XCTL,
 ATTACH, OR LOAD SERVICE ROUTINES. THE AUTHORIZED ROUTINE
 REQUESTED
 BUT A
 LIBRARY.
 THE***
 A MODULE WHICH COULD NOT BE FOUND ON AN AUTHORIZED LIBRARY,
 COPY OF THE MODULE MAY HAVE BEEN FOUND ON A NONAUTHORIZED
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S306 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S306-RC)**
 ...
 LOAD MODULE WHICH WAS LOADED FROM AN AUTHORIZED LIBRARY BY
 AN
 UNAUTHORIZED CALLER.  ANOTHER COPY OF THE MODULE COULD NOT
 BE
 FOUND.
...
WAS
 AREA TO
 THE ERROR OCCURRED DURING EXECUTION OF AN R-FORM FREEMAIN
 MACRO FOR ONE OF THE FOLLOWING REASONS:- A FREEMAIN FOR AN ENTIRE SUBPOOL WAS REQUESTED (REGISTER 1
 ZERO) BUT THE LENGTH IN REGISTER 0 WAS NOT SET TO ZERO.- THE SPECIFIED SUBPOOL COULD NOT BE FOUND.- THE SP PARAMETER WAS SPECIFIED, BUT THE VIRTUAL STORAGE
 BE RELEASED WAS NOT WITHIN THE SUBPOOL SPECIFIED.- THE SP PARAMETER WAS NOT SPECIFIED, BUT THE VIRTUAL
 STORAGE AREA
 TO BE RELEASED WAS NOT WITHIN SUBPOOL ZERO.- THE SP PARAMETER WAS SPECIFIED CORRECTLY, BUT THE
 BOUNDARIES OF
 THE STORAGE AREAS TO BE FREED WERE NOT COMPLETELY DESCRIBED
 BY
 THE***
 A DECSRIPTOR QUEUE ELEMENT (DQE).
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S30A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S30A-RC)**
...
S30A - 0C - CSA STORAGE IS NOT IN THE SUBPOOL SPECIFIED.
 ...
 OR THE FREEMAIN START ADDRESS IS NOT VALID.
 S313 -      
AN ABEND OCCURRED WHILE OPENING A DASD DATA SET BECAUSE
 OF A VTOC I/O ERROR OR A SYSTEM LOGIC ERROR.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S313 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S313-RC)**
 ...
 SERVICES (DCS) WHILE TRYING TO GET A SYSTEM-DETERMINED
 BLOCKSIZE.
 S314 -      
THE***
 I/O ERROR IN READING DSCB
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S314 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S314-RC)**--POSSIBLE CAUSE--
 ...
 EXECUTION OF A CLOSE MACRO AND STANDARD USER LABELS WERE
 SPECIFIED.
 S317 -      
FILE.
 THE***
 AN OCCURRED DURING A BSAM CLOSE TYPE=T MACRO FOR A DASD
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S317 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S317-RC)**
 ...
 MACRO INSTRUCTION DID NOT HAVE THE EXPECTED FORMAT-1 OR
 FORMAT-4
 FORMAT ID.
 S322 -      
S32D -      
INCORRECT
 S337 -      
THE***
 JOB OR STEP TIME EXCEEDED THE SPECIFIED LIMIT
 PROGRAM IS IN A LOOP
 INSUFFICIENT TIME PARAMETER ON JOB OR EXEC CARD
 ADDRESS IN SEGMENT TABLE OR ENTRY TABLE OUTSIDE OF STORAGE
 FOR JOB STEP
 AN OVERLAY PROGRAM WAS FOUND TO CONTAIN A RECORD OF
 LENGTH, OR AN I/O ERROR WAS ENCOUNTERED- RE-LINKEDIT IT
 THE ERROR OCCURRED WHEN THE END OF A DATA SET WAS REACHED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S337 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S337-RC)**--POSSIBLE CAUSE--
...
SUBTASK
 S378 -      
THE***
 AN UNAUTHORIZED TASK ATTEMPTED TO USE AUTHORIZED OPTIONS OF
 THE ENQ MACRO INSTRUCTION.
 A DETACH MACRO INSTRUCTION SPECIFYING THE STAE=YES OPERAND
 HAS BEEN ISSUED BY THE ORIGINATING TASK, BUT THE SPECIFIED
 SUBTASK HAS NOT COMPLETED EXECUTION.--POSSIBLE CAUSE--
 THIS MAY OR MAY NOT BE AN ERROR.  IF YOU INTEND FOR THE
 ...
 THE ERROR OCCURRED DURING EXECUTION OF AN RC OR RU FORM
 FREEMAIN MACRO.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S378 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S378-RC)**--POSSIBLE CAUSE--
 A FREEMAIN FOR AN ENTIRE SUBPOOL WAS REQUESTED (REGISTER 1
 WAS
 TO BE
 AREA
 OF
 ...
 S400 -      
THE DCB IN THE DEB DOES NOT EQUAL THE DCB ADDRESS IN THE
 IOB.
 THE
 TO
 DETERMINED
...
IN THE ABNORMAL TERMINATION DUMP, THE TCB FIELD TCBEXCPD (AT
 OFFSET C0) POINTS TO THE EXCP PROBLEM DETERMINATION AREA.
 ITEMS IN THE PROBLEM DETERMINATION AREA OF GREATEST INTEREST
 YOU ARE: (ALL OFFSETS IN HEX)- OFFSET 10 CONTAINS A COPY OF THE REGISTERS WHEN EXCP
 THE ERROR CONDITION.- OFFSET 50 CONTAINS THE CONTENTS OF THE REQUEST QUEUE
 (RQE) IF ALLOCATED, WHEN THE PROGRAM CHECK OCCURRED.
 THE EVENTS POST MACRO INSTRUCTION ENCOUNTERED AN ERROR WHILE
 PROCESSING AN EVENT CONTROL BLOCK (ECB).
THE***
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S402 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S402-RC)**
...
S40A -      
THE***
 THE REQUESTED LOAD MODULE WAS MARKED BY THE LINKAGE EDITOR
 AS ONLY LOADABLE.
 AN R-FORM FREEMAIN MACRO IS ATTEMPTING TO FREE AN ENTIRE
 SUBPOOL THAT SHOULD NOT BE FREEMAINED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S40A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S40A-RC)**
 S40A - 04 - ATTEMPTING TO FREEMAIN A SUBPOOL THAT CANNOT BE SUBPOOL
 FREEMAINED.
 S40A - 08 - A  PROBLEM PROGRAM IS TRYING TO FREE SUBPOOL ZERO.
 S40D -      
THIS COMPLETION CODE IS FOUND ONLY IN ASCBS.  BECAUSE OF
 RECURSIVE ERRORS TRYING TO TERMINATE A TASK IT BECAME
 NECESSARY
 ADDRESS
 S413 -      
THE***
 TO TERMINATE THE MEMORY. THE RTM ATTEMPTS TO TERMINATE THE
 SPACE. A DUMP MAY HAVE BEEN TAKEN TO THE SYS1.DUMP DATASET.
 INPUT, INOUT, OR RDBACK SPECIFIED, BUT NO VOL SER IN DD; I/O
 ERROR IN READING VOLUME LABEL; COULD NOT MOUNT VOLUME ON
 DEVICE; VOLUMES SPECIFIED LESS THAN DEVICES ALLOCATED.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S413 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S413-RC)**--POSSIBLE CAUSE--
 ...
 TRIED TO OPEN SECOND DATA SET ON A TAPE WITHOUT CLOSING THE
 FIRST
 GIVING
 THE REQUESTED DRIVE WAS SWITCHED OFF
 SORT VERB BLEW DUE TO SD NOT MATCHING FD WITH USING OR
 S413 - 04 - NO UNIT IS AVAILABLE FOR MOUNTING THE VOLUME FOR THE
 DATASET BEING OPENED.  THE VOLUME ALREADY ON THE ALLOCATED
 UNIT AS
 SPECIFIED BY THE VOL=SER IS EITHER PERMANENTLY RESIDENT OR
 RESERVED. IT COULD NOT BE DEMOUNTED IN ORDER TO MOUNT THE
 REQUIRED
 DCB IS
 THE
 ABNORMAL
 VOLUME, OR THE VOLUME CANNOT BE DEMOUNTED BECAUSE ANOTHER
 OPEN ON THAT DEVICE OR THE DEVICE TYPE IS INCOMPATIBLE WITH
 DSORG IN THE DCB. THIS ERROR MAY BE DUE TO A PREVIOUS
...
S413 - 0C - AN I/O ERROR OCCURRED READING THE VOLUME LABEL ON A
 ...
 SEQUENCE NUMBER ON THE ASSOCIATED DD STATEMENT WAS GREATER
 THAN
 ...
 INCOMPATIBLE WITH THE RECORDING DENSITY OF THE DRIVE
 ALLOCATED
 TO THE DATASET.
 S413 - 28 - NON-ZERO RETURN CODE FROM OPEN DAVV (DIRECT ACCESS VOLUME
 VERIFICATION) MODULE AFTER A IEFSSREQ MACRO INSTRUCTION.
 THIS
 LOGICAL
 CODE INDICATES THE SUBSYSTEM DISCOVERED A FUNCTIONAL OR
 ...
 WAS WRITTEN WHEN THE RULES COULD NOT BE ENFORCED.  FOR
 OUTPUT,
 THE ABEND PREVENTS ANY VIOLATION FROM HAPPENING.
 S414 -      
THE***
 AN ERROR OCCURRED DURING CLOSE PROCESSING FOR A DATASET ON A
 DIRECT ACCESS DEVICE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S414 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S414-RC)**
 ...
 AN I/O ERROR OCCURRED WRITING AN UPDATED FORMAT-1 DSCB
 DURING
 THE EXECUTION OF A BSAM CLOSE TYPE=T MACRO INSTRUCTION.
 S437 -      
THE***
 AN ERROR OCCURRED DURING END-OF-VOLUME.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S437 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S437-RC)**
 S437 - 08 - WHEN FEOV WAS ISSUED, THE DCB POINTER WAS INVALID.
 S437 - 0C - DEB NOT IN DEB TABLE.
 S478 -      
AN
 AN RC-FORM OR RU-FORM FREEMAIN MACRO IS ATTEMPTING TO FREE
 ENTIRE SUBPOOL THAT SHOULD NOT BE FREEMAINED.
THE***
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S478 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S478-RC)**
 S478 - 04 - ATTEMPTING TO FREEMAIN A SUBPOOL THAT CANNOT BE SUBPOOL
 FREEMAINED.
 S478 - 08 - A  PROBLEM PROGRAM IS TRYING TO FREE SUBPOOL ZERO.
 S502 -      
DURING EXECUTION OF A POST MACRO INSTRUCTION, THE CONTROL
 PROGRAM FOUND THAT THE EVENTS TABLE SPECIFIED IN THE ECB HAD
 NO
 ...
 S513 -      
THE***
 THE ERROR OCCURRED DURING EXECUTION OF AN OPEN MACRO
 INSTRUCTION FOR A DATA SET ON MAGNETIC TAPE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S513 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S513-RC)**
 S513 - 04 - AN OPEN MACRO WAS ISSUED FOR A MAGNETIC TAPE DATASET
 ALLOCATED TO A DEVICE THAT ALREADY HAS AN OPEN DATASET ON
 IT.
...
JOB OR TSO SESSION EXCEEDED MAXIMUM JOB WAIT TIME OR
 OPERATOR DID NOT MOUNT THE REQUIRED TAPE WITHIN ALLOWED TIME
 LIMIT
 S582 - 00 - (00 IS THE ONLY POSSIBLE RETURN CODE ASSOCIATED WITH S582)
 WHILE PERFORMING RACHECK SVC PROCESSING, RACF WAS UNABLE TO
 VERIFY
 A USER BECAUSE NO ACCESS CONTROL ENVIRONMENT (ACEE) WAS
 AVAILABLE.
 RACF
 S604 -      
THE***--POSSIBLE CAUSE--
 USER LOGGED ON OR JOB STARTED WHILE RACF WAS INACTIVE, BUT
 HAS SINCE BEEN REACTIVATED.
 GETMAIN ERROR--POSSIBLE CAUSE--
 MISSING SORTLIB CARD
 SUBSCRIPT ERROR - CLOBBERED FREE QUEUE ELEMENT
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S604 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S604-RC)**
 ...
 S605 -      
FREEMAIN ERROR--POSSIBLE CAUSE--
 MISSING SORTLIB CARD
 SUBSCRIPT ERROR - CLOBBERED FREE QUEUE ELEMENT
THE***
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S605 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S605-RC)**
 ...
 S60A -      
(NON-MVS/XA) FREEMAIN QUEUE ELEMENT OR ADDRESS ERROR
 NEEDS MORE CORE
 JCL FOR INPUT DISKS REQUESTED 2 UNITS, GAVE 1 VOLUME SERIAL
 FORGOT DD CARD FOR SYS1.SORTLIB
 S613 -      
THE***
 I/O ERROR IN TAPE POSITIONING OR LABEL PROCESSING
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S613 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S613-RC)**--POSSIBLE CAUSE--
 TAPE LABEL WAS NOT IN REQUESTED DENSITY
 DCB MISSING
 DEN MISSING ON A TAPE THAT REQUIRES IT DEFAULT DEN=3 (1600
 BPI)
 ...
 ASSOCIATED WITH THE SAME TAPE SINCE IT WAS LAST MOUNTED,
 POSSIBLY
 IN A PREVIOUS JOB OR STEP, LEAVING THE TAPE POSITIONED
 IMPROPERLY.
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S614 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S614-RC)**--POSSIBLE CAUSE--
 ...
 A DIRECT ACCESS DEVICE DURING EXECUTION OF A CLOSE MACRO.--POSSIBLE CAUSE--
 OPERATOR DISMOUNTED VOLUME WHILE PROGRAM WAS PROCESSING
 S614 - 08 - A FILE MARK SHOULD HAVE BEEN WRITTEN ON AN OUTPUT DATASET.
 THE DCBFDAD FIELD IN THE DCB INDICATED AN EXTENT NUMBER IN
 THE
...
COMPLETED
 S637 -      
THE***
 TSO SESSION WAS CANCELLED
 OPERATOR STOPPED TSO OR CANCELLED THE USER
 USER SIGNALLED ATTN AFTER THE ALLOCATION PROCESS HAD
 THE USER DISCONNECTED HIS TERMINAL FROM THE SYSTEM
 I/O ERROR IN WRITING TAPE MARK, TAPE POSITIONING, READING
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S637 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S637-RC)**
 ...
 DATASET THAT WAS OPENED WITH THE OPTION INPUT OR INOUT TO BE
 READ
 BACKWARD. IF IT IS A TAPE WITH STANDARD LABELS, THE ERROR
 OCCURRED
 THE
 POSITIONING AT THE LABELS.  IF IT IS A TAPE WITH NO LABELS,
 ...
 ISSUED FOR A DATASET WITH DISP=PASS AND NO OPEN OPTION 2
 SPECIFIED.
 ...
 VOLUME.  EITHER FEOV WITH A REWIND OPTION WAS ISSUED, OR NO
 OPEN
 ...
 THAT WAS OPENED WITH THE OPTION INPUT OR INPUT TO BE READ
 FORWARD.
...
S684 -      
THE MODULE CALLING THE RACF MANAGER OR THE RACLIST FUNCTION
 IS NOT AUTHORISED TO THE SVC FUNCTION.THE MODULE CALLING THE
 RACF
 0-7, OR
 FUNCTION
 VERIFY
...
S713 -      
THE***
 MANAGER MUST BE AUTHORISED (APF-AUTHORISED, IN SYSTEM KEY
 IN SUPERVISOR STATE).  THE MODULE CALLING THE RACLIST
 MUST BE AUTHORISED (APF-AUTHORISED, IN SYSTEM KEY 0-7, OR IN
 SUPERVISOR STATE) OR IN THE RACF-AUTHORISED CALLER TABLE AND
 FETCHED FROM AN AUTHORISED LIBRARY.- POSSIBLE USER ERROR.  IF THE RACF MANAGER WAS CALLED,
 ...
 A PROGRAM CHECK OCCURRED DURING EXCP PROCESSING OF AN I/O
 REQUEST.  THE PROGRAM CHECK OCCURRED IN A SUPERVISOR SERVICE
 ROUTINE CALLED BY EXCP. THIS IS LIKELY NOT A PROGRAMMER
 IN THE ABNORMAL TERMINATION DUMP, THE TCB FIELD TCBEXCPD (AT
 OFFSET C0) POINTS TO THE EXCP PROBLEM DETERMINATION AREA.
 ITEMS IN THE PROBLEM DETERMINATION AREA OF GREATEST INTEREST
 YOU ARE: (ALL OFFSETS IN HEX)- OFFSET 2 CONTAINS A FLAG BYTE INDICATING WHERE THE ERROR
 OCCURRED.
 ...
 .... ....  ERROR IN SIO APPENDAGE- OFFSET 4 CONTAINS A COPY OF THE PROGRAM STATUS WORD (PSW)
 BEFORE RTM WAS ENTERED.- OFFSET E CONTAINS THE INTERRUPTION CODE.- OFFSET 10 CONTAINS A COPY OF THE REGISTERS WHEN EXCP
 THE ABEND MACRO.- OFFSET 54 CONTAINS THE CONTENTS OF THE REQUEST QUEUE
 (RQE), IF ALLOCATED, FOR THE I/O REQUEST BEING PROCESSED.
 THE REQUESTED LOAD MODULE WAS MARKED BY THE LINKAGE EDITOR
 AS NOT EXECUTABLE.
 BAD PRIOR LINKEDIT - CHECK THE LKED SYSPRINT
 TRIED TO WRITE ON A DATASET WHOSE DATE PROTECTION HAD NOT
 EXPIRED. OPERATOR REPLIED 'M' TO IEC507D TO HONOR EXP DATE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S713 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S713-RC)**
 S713 - 04 - A DATASET ON MAGNETIC TAPE WAS OPENED FOR INOUT, OUTIN,
 OUTPUT, OUTINX, OR EXTEND, BUT THE VOLUME CONTAINED A
 DATASET
 WHOSE EXPIRATION DATE HAD NOT BEEN REACHED.
S713 - 08 - AN OPEN MACRO WAS ISSUED WITH INOUT FOR A DATASET ON A
 DIRECT ACCESS DEVICE WITH DISP=OLD SPECIFIED ON THE DD
 STATEMENT.
 ...
 S714 -      
THE***
 I/O ERROR IN LABEL PROCESSING - CLOSE FOR MAGNETIC TAPE
 TAPE DRIVE FAILURE DURING CLOSE
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S714 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S714-RC)**
...
TYPE=T
 ERROR.
 THE***
 AN I/O ERROR OCCURRED DURING EXECUTION OF A BSAM CLOSE
 MACRO INSTRUCTION.  THE RETURN CODE INDICATES THE EXACT
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S717 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S717-RC)**
 ...
 S722 -      
THE OUTPUT LIMIT SPECIFIED BY THE OUTLIM KEYWORD ON THE
 SYSOUT
 STATEMENT
 S737 -      
THE***
 DD STATEMENT OR BY THE LINES KEYWORD ON THE JOBPARM DD
 WAS EXCEEDED.
 I/O ERROR; DSCB NOT FOUND FOR MULTI-VOLUME OR CONCATENATED
 DATA SET.
 BE SURE ALL THE DATA SETS EXIST FOR CONCATENATED DATA SETS
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S737 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S737-RC)**
 S737 - 04 - AN I/O ERROR OCCURRED READING A DSCB DURING END OF VOLUME
 PROCESSING OR THE DSCB COULD NOT BE FOUND ON THE SPECIFIED
 VOLUME.
...
DCBFDAD
 ATTEMPTING TO WRITE A FILE MARK AT THE END OF DATA.  THE
 FIELD IN THE DCB INDICATED AN EXTENT NUMBER IN THE DEB
 GREATER
 COULD
 WRITTEN.
 THAN THE NUMBER OF EXTENTS IN THE DATASET. CONSEQUENTLY IT
 NOT BE DETERMINED WHERE THE FILE MARK SHOULD HAVE BEEN
...
S804 -      
THE***
 AN UNEXPECTED ERROR OCCURRED DURING THE EXECUTION OF AN ENQ
 MACRO. RELATED INFORMATION IS RECORDED IN SYS1.LOGREC.
 AN ERROR OCCURRED WHEN EXCP ATTEMPTED TO FIX A PAGE FOR
 THIS EXCP REQUEST.
 REQUEST FOR 0 BYTES OF VIRTUAL STORAGE OR NOT ENOUGH VIRTUAL
 STORAGE AVAILABLE FOR GETMAIN REQUEST.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S804 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S804-RC)**--POSSIBLE CAUSE--
 ...
 S806 -      
THE***
 BLDL DETECTED ERROR: MODULE NOT FOUND OR I/O ERROR DURING
 DIRECTORY SEARCH
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S806 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S806-RC)**--POSSIBLE CAUSE--
 MISSING JOBLIB OR STEPLIB CARD
 TRIED TO EXECUTE A NON-EXISTENT PROGRAM
 TRIED TO EXECUTE IN BATCH A PROGRAM ASSEMBLED WITH 'TEST'
 OPTION
 ...
 CONTROL PROGRAM ROUTINE ATTEMPTED TO SEARCH THE DIRECTORY OF
 THE
 ...
 SPECIFIED ENTRY POINT, BUT THE DCB OF THE LIBRARY WAS NOT
 OPEN.
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S80A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S80A-RC)**
 ...
 AN OPEN MACRO WAS ISSUED FOR A DATASET ON MAGNETIC TAPE, BUT
 THE
 DATASET NAME ON THE HEADER LABEL DID NOT MATCH THAT IN THE
 JFCB.
 S822 -      
ONE
 DEFERRED
 ...
 DEPENDING UPON WHETHER THE JOB WAS AN ORDINARY JOB OR A
 CHECKPOINT RESTART:- IEF085I REGION UNAVAILABLE, ERROR CODE = CDE- IEF186I REGION UNAVAILABLE FOR RESTART, ERROR CODE = CDE
 ***IF THE IEF085I OR IEF186I ERROR CODE IS AVAILABLE, SUPPLY
 ***IN THE S822 OPERAND FOR MORE INFORMATION: H ABEND
 O(S822-EC)***
 S822 - 08 - IEF085I - A V=V REGION WAS REQUESTED AND A REGION SIZE WAS
 SPECIFIED WHICH WAS LARGER THAN THE PRIVATE AREA, OR A V=R
 REGION
 WAS REQUESTED AND A REGION SIZE GREATER THAN THE V=R AREA
 WAS
 REGION
 CHANGED
 SPECIFIED.
 IEF186I - THE REGION PARAMETER WAS INCREASED SO THAT THE
 COULD NOT BE ALLOCATED OR THE CONFIGURATION OF THE SYSTEM
 SO THAT THE REGION COULD NOT BE OBTAINED:- FOR ADDRSPC=REAL, THE SIZE OF THE REAL AREA WAS DECREASED.- FOR ADDRSPC=VIRT, THE SIZE OF THE PRIVATE AREA DECREASED
 ...
 OR DAMAGED PAGES IN THE V=R AREA MADE IT IMPOSSIBLE TO
 OBTAIN THE
 REQUESTED REGION, OR A V=R REGION WAS REQUESTED AND THERE
 WAS NOT
 ENOUGH SQA AVAILABLE FOR THE SYSTEM TO COMPLETE THE
 PROCESSING OF
 THE REQUEST.
 IEF186I - IF A REAL REGION WAS REQUESTED, EITHER LONG-FIXED
 OR
THE
 DAMAGED PAGES IN THE REAL AREA MADE IT IMPOSSIBLE TO OBTAIN
 REQUIRED REGION.
 ...
 S837 -      
THE***
 THE ERROR OCCURRED DURING END OF VOLUME FOR A SEQUENTIAL
 DATA SET.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S837 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S837-RC)**
 ...
 OPEN TYPE=J WAS USED TO CHANGE VOLUME SERIAL BUT NOT WRITTEN
 BACK
 ...
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***S878 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S878-RC)**
 ...
 AREA TO BE FREED BY FREEMAIN IS NOT ON A DOUBLEWORD
 BOUNDARY.
 S90A - 04 - (04 IS THE ONLY POSSIBLE RETURN CODE ASSOCIATED WITH S90A)
 THE ERROR OCCURRED DURING THE EXECUTION OF AN R-FORM
 FREEMAIN
 MACRO.  THE ADDRESS OF THE STORAGE AREA TO BE RELEASED WAS
 NOT ON
 S913 -      
THE***
 A DOUBLEWORD BOUNDARY (A MULTIPLE OF 8).
 THE ERROR OCCURRED DURING 1) THE EXECUTION OF AN OPEN MACRO
 OR DURING END-OF-VOLUME PROCESSING FOR A PASSWORD PROTECTED
 DATASET, OR 2) THE EXECUTION OF AN OPEN MACRO INVOLVING A
 CHECKPOINT DATASET.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***S913 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(S913-RC)**--POSSIBLE CAUSE--
DATASET
 OPERATOR FAILED TO SUPPLY CORRECT PASSWORD FOR PROTECTED
 UNAUTHORIZED ATTEMPT TO USE LABEL=BLP
 PREVIOUS DATASET ON THIS TAPE WAS PROTECTED, NEW DATASET IS
 NOT
 S913 - 04 - AN OPEN MACRO WAS ISSUED FOR A MAGNETIC TAPE DATASET WITH
 AMERICAN NATIONAL STANDARD LABELS.  THE VOLUME ACCESSABILITY
 BYTE
 (OFFSET X'0A') IN THE VOLUME LABEL IS NOT BLANK.  THIS
 INDICATES
 IT WAS
 THE LABEL WAS NOT WRITTEN FOR USE ON AN IBM SYSTEM, OR THAT
 WRITTEN BY THE USER. THE VOLUME MUST BE RECREATED.
 S913 - 08 - AN OPEN MACRO WAS ISSUED FOR A MAGNETIC TAPE DATASET WITH
 AMERICAN NATIONAL STANDARD LABELS. THE SECURITY BYTE IN THE
 HEADER
 LABEL WAS NOT BLANK AND NOT EQUAL TO X'F1'. THIS MEANS THE
 LABEL
 THE
 EITHER WAS NOT CREATED ON AN IBM SYSTEM OR WAS CREATED BY
 ...
 PROCESSING) AN THE TASK WAS NOT AUTHORIZED. BLP MAY BE
 SPECIFIED
 IN THE JCL (IF THE INSTALLATION READER PROCEDURE ALLOWS IT),
 BUT
 IS
 THE JFCB MAY NOT BE MODIFIED TO INDICATE BLP UNLESS THE TASK
 AUTHORIZED.
 S913 - 1C - THE ERROR OCCURRED DURING THE EXECUTION OF AN OPEN TYPE=J
 MACRO TO A DIRECT ACCESS DEVICE DATASET. THE DATASET NAME
 SUPPLIED
 TO OPEN WAS NOT AVAILABLE TO THE JOB. EITHER THE DATASET WAS
 BEING
 IT,
 INPUT
 USING
 OPENED FOR INPUT AND SOME OTHER JOB HAD EXCLUSIVE CONTROL OF
 OR THE DATASET WAS BEING OPENED FOR AN OPTION OTHER THAN
 (THUS REQUIRING EXCLUSIVE CONTROL) AND SOME OTHER JOB WAS
 THE DATASET.
 S913 - 20 - AN OPEN MACRO USING THE EXCP ACCESS METHOD WAS ISSUED IN
 WHICH USER-WRITTEN APPENDAGES WERE REQUIRED. THE APPENDAGE
 NAMES
 WERE NOT INCLUDED IN THE PARMLIB MEMBER IEAAPP00, AND THE
 PROGRAM
 ISSUING THE OPEN WAS NOT AUTHORIZED (BY APF OR PROTECT KEY).
 S913 - 28 - AN OPEN MACRO WAS ISSUED FOR A CHECKPOINT DATASET.  THE
 DATASET ORGANIZATION WAS NOT BPAM OR BSAM AND THE TASK WAS
 NOT
 AUTHORIZED VIA THE AUTHORIZED PROGRAM FACILITY (APF).
 S913 - 2C - AN OPEN MACRO WAS ISSUED TO AN ISAM DATASET DEFINED BY TWO
 OR THREE DD STATEMENTS.  EITHER THE DATASET NAMES CODED IN
 THE
OPEN
 DD STATEMENTS WERE NOT ALL THE SAME, OR THE JFCB PASSED TO
 (IF AN OPEN TYPE=J IS BEING USED) HAS A DATASET NAME
 DIFFERENT
 FROM THAT CODED IN THE DD STATEMENTS.
 S913 - 30 - AN OPEN MACRO WAS ISSUED TO WRITE A DATASET ON A MAGNETIC
 TAPE CONTAINING ONE OR MORE PREVIOUS DATASETS. THE
 PROTECTION
 ...
 THE FORMAT-1 DSCB IN THE VTOC) DATA SET, BUT THE DATA SET IS
 NOT
 DEFINED TO RACF.
 ...
 SPECIFIED NAME.  THIS NAME AND FILE SEQUENCE INDICATOR DO
 NOT
 ...
 AUTHORIZED TO DEFINE A DATASET PROTECTED BY A GENERIC
 PROFILE.
 S913 - 70 - THE USER HAS RACF EXECUTE ACCESS ONLY AND ONE OF THE
 FOLLOWING REQUIREMENTS WAS VIOLATED:- THE CALLER MUST BE IN SUPERVISOR STATE
 OR THE DATA SET MUST BE A PDS.- THE ACCESS METHOD MUST NOT BE EXCP.
...
POINTS
 DEBUGGING
 A PROGRAM CHECK OCCURRED IN AN APPENDAGE. IN THE ABNORMAL
 TERMINATION DUMP, THE TCB FIELD TCBEXCPD (AT OFFSET C0)
 TO THE EXCP PROVIDED DEBUGGING AREA. THE ITEMS IN THE
 AREA OF GREATEST INTEREST TO YOU ARE: (ALL OFFSETS IN HEX)- OFFSET 2 CONTAINS A FLAG BYTE INDICATING WHERE THE ERROR
 OCCURRED.
THE BIT PATTERN IS AS FOLLOWS:
 1... ....  ERROR IN SVC PORTION OF EXCP
 .1.. ....  ERROR IN SRB PORTION OF EXCP
...
THE***
 THE ABEND MACRO.- OFFSET 54 CONTAINS THE CONTENTS OF THE REQUEST QUEUE
 (RQE), IF ALLOCATED, FOR THE I/O REQUEST BEING PROCESSED.
 ERROR DURING FREEMAIN.  AREA TO BE RELEASED OVERLAPS FREE
 STORAGE OR PAGE-FIXED STORAGE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SA05 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SA05-RC)**
 ...
 SA0A -      
ERROR DURING R-FORM FREEMAIN.  AREA TO BE RELEASED OVERLAPS
 FREE STORAGE OR PAGE-FIXED STORAGE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***SA0A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SA0A-RC)**
...
MULTIFILE
 ...
 I/O ERROR OCCURRED DURING A CLOSE ATTEMPTING A PARTIAL
 RELEASE
...
TASK
 IN
 THE***
 THE ERROR OCCURRED DURING END OF VOLUME PROCESSING.  THE
 IS TERMINATED UNLESS THE ERROR IS TO BE IGNORED AS SPECIFIED
 THE DCB ABEND EXIT ROUTINE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SA37 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SA37-RC)**--POSSIBLE CAUSE--
 AN OPEN DCB MAY HAVE BEEN PARTIALLY OVERLAID
 THE DCB MAY HAVE BEEN CLOSED IN A SYNAD ROUTINE
 THE DCB MAY HAVE BEEN AUTOMATICALLLY CLOSED BY A PREVIOUS
 END OF
 EXIT
 OF VOLUME ERROR WHERE IGNORE WAS SPECIFIED IN THE DCB ABEND
 ...
 SA78 -      
ERROR DURING RU-FORM FREEMAIN.  AREA TO BE RELEASED OVERLAPS
 FREE STORAGE OR PAGE-FIXED STORAGE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***SA78 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SA78-RC)**
...
ABNORMAL
 POINTS
 A PROGRAM CHECK OCCURRED IN AN EXCP PROCEDURE. IN THE
 TERMINATION DUMP, THE TCB FIELD TCBEXCPD (AT OFFSET C0)
DEBUGGING
 TO THE EXCP PROVIDED DEBUGGING AREA. THE ITEMS IN THE
 AREA OF GREATEST INTEREST TO YOU ARE: (ALL OFFSETS IN HEX)- OFFSET 2 CONTAINS A FLAG BYTE INDICATING WHERE THE ERROR
...
PROBLEM
 PERFORM
 SB13 -      
THE ABEND MACRO.- OFFSET 50 CONTAINS THE TRANSLATION EXCEPTION ADDRESS,
 IF APPLICABLE.- OFFSET 54 CONTAINS THE CONTENTS OF THE REQUEST QUEUE
 (RQE), IF ALLOCATED, FOR THE I/O REQUEST BEING PROCESSED.- OFFSET 80 IS THE START OF THE 160 BYTE BLOCKS INVOLVED IN
 I/O REQUEST, IF ALLOCATED.
 THE ERROR OCCURRED DURING EXECUTION OF A GETMAIN MACRO:- A SUBPOOL NUMBER GREATER THAN 127 WAS SPECIFIED BY A
 PROGRAM (A PROGRAM NOT AUTHORIZED TO USE VALID SUBPOOL
 NUMBERS GREATER THAN 127) (RC=08)- AN AUTHORIZED PROGRAM REQUESTED AN INVALID SUBPOOL (RC=04)--POSSIBLE CAUSE--
 TRIED TO CHANGE BLOCKING FACTOR WHILE LOADING A DATASET
 COBOL - CONTROL PASSED BEYOND END OF PROGRAM DUE TO INVALID
 THE ERROR OCCURRED DURING EXECUTION OF A FREEMAIN MACRO:- A SUBPOOL NUMBER GREATER THAN 127 WAS SPECIFIED BY A
 PROGRAM (A PROGRAM NOT AUTHORIZED TO USE VALID SUBPOOL
 NUMBERS GREATER THAN 127) (RC=08)- AN AUTHORIZED PROGRAM REQUESTED AN INVALID SUBPOOL (RC=04)--POSSIBLE CAUSE--
 TRIED TO CHANGE BLOCKING FACTOR WHILE LOADING A DATASET
 COBOL - CONTROL PASSED BEYOND END OF PROGRAM DUE TO INVALID
 THE ERROR OCCURRED DURING EXECUTION OF AN R-FORM GETMAIN OR
 FREEMAIN MACRO:- A SUBPOOL NUMBER GREATER THAN 127 WAS SPECIFIED BY A
 PROGRAM (A PROGRAM NOT AUTHORIZED TO USE VALID SUBPOOL
 NUMBERS GREATER THAN 127) (RC=08)- AN AUTHORIZED PROGRAM REQUESTED AN INVALID SUBPOOL (RC=04)--POSSIBLE CAUSE--
 TRIED TO CHANGE BLOCKING FACTOR WHILE LOADING A DATASET
 COBOL - CONTROL PASSED BEYOND END OF PROGRAM DUE TO INVALID
 THE ERROR OCCURRED DURING THE EXECUTION OF AN OPEN MACRO FOR
A DATA SET ON A PRINTER WITH UNIVERSAL CHARACTER SET (UCS).
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 THE***
 ***SB13 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SB13-RC)**
 ...
 BLDL FOUND A PERMANENT I/O ERROR WHEN SEARCHING
 SYS1.IMAGELIB
 FOR THE REQUESTED UCS IMAGE.
 SB13 - 0C - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER
 OR TO A 3886.  A PERMANENT I/O ERROR PERSISTED AFTER TWO
 ATTEMPTS
 AT LOADING THE UCS BUFFER -OR- A PERMANENT I/O ERROR
 OCCURRED
 WHEN LOADING THE FORMAT RECORD INTO THE 3886.
 SB13 - 10 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER.
 A PERMANENT I/O ERROR RESULTED WHEN TRYING TO PRINT THE
 CHARACTER
 SET IMAGE FOR VISUAL VERIFICATION, OR DURING AN OPERATION
 WHICH
 RESULTED IN POSSIBLE LOST DATA.
 SB13 - 14 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER.
 THE OPERATOR REPLIED 'CANCEL' TO IEC121D AFTER THE CHARACTER
 SET
 ...
 THE OPERATOR REPLIED 'CANCEL' TO IEC128D AFTER THE FCB IMAGE
 WAS
 ...
 OR A PERMANENT I/O ERROR OCCURRED WHILE READING
 SYS1.IMAGELIB.
 SB13 - 24 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER,
 BUT THE VOLUME CONTAINING SYS1.IMAGELIB WAS NOT MOUNTED OR
 WAS
 NOT CATALOGUED.  DURING 3886 OPEN, THE FORMAT RECORD
 IDENTIFIER
 WAS NOT SPECIFIED.
 SB13 - 28 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER,
 BUT A PERMANENT I/O ERROR PERSISTED WHILE OPENING
 SYS1.IMAGELIB.
 ...
 A PERMANENT I/O ERROR PERSISTED AFTER TWO ATTEMPTS AT
 LOADING THE
 FORMS CONTROL BUFFER (FCB).
 SB13 - 34 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER.
 THE OPERATOR REPLIED 'CANCEL' TO IEC127D BECAUSE THE FCB
 IMAGE
 COULD NOT BE FOUND IN SYS1.IMAGELIB.
SB13 - 38 - AN OPEN WAS ISSUED TO A DATA SET ALLOCATED TO A PRINTER.
 NO OPERATION WAS PERFORMED FOR ONE OF THE FOLLOWING REASONS:- THE DATA CONTROL BLOCK (DCB) WAS NOT OPEN.- THE DCB WAS NOT VALID FOR A SEQUENTIAL DATA SET.- THE SETPRT PARAMETER LIST WAS NOT VALID.- THE OUTPUT DEVICES WAS NOT A UCS OR 3800 PRINTER.
 SB14 -      
THE***
 AT CLOSE A PARTITIONED DATA SET DIRECTORY CANNOT BE UPDATED
 BECAUSE THE NAME IS ALREADY IN THE DIRECTORY, NO SPACE IS
 AVAILABLE IN THE DIRECTORY, OR AN I/O ERROR WAS ENCOUNTERED
 IN THE DIRECTORY.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SB14 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SB14-RC)**--POSSIBLE CAUSE--
 ALLOCATED DIRECTORY SPACE IN JCL, WRITTEN AS SEQUENTIAL DATA
 SET
...
SYSTEM
 THE***
 AT END OF VOLUME, THE VOLUME MUST BE DEMOUNTED, BUT THE
 IS UNABLE TO DISMOUNT THE VOLUME.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SB37 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SB37-RC)**--POSSIBLE CAUSE--
 ...
 A VOLUME IN ORDER TO MOUNT THE NEXT VOLUME OF THE DATASET.
 IT WAS
 UNABLE TO DEMOUNT THE VOLUME FOR ONE OF THE FOLLOWING
 REASONS:- THE VOLUME WAS PERMANENTLY RESIDENT OR WAS RESERVED.- ANOTHER JOB HAD DATASETS ALLOCATED ON THE VOLUME.- THE FAILING TASK HAD OPEN DATASETS ON THE VOLUME.
 FOR AN OUTPUT DATASET ON A DIRECT ACCESS DEVICE:- NO MORE SPACE WAS AVAILABLE ON THE VOLUME.- THE DATASET ALREADY HAD 16 EXTENTS BUT REQUIRED MORE
 SPACE.- MORE SPACE WAS REQUIRED, BUT THE VTOC WAS FULL.
 FOR AN OUTPUT DATASET ON MAGNETIC TAPE:- A VOLUME NEEDED TO BE DEMOUNTED BECAUSE THE REFLECTIVE
 SPOT
 ...
 EXTEND THE DATASET TO A VOLUME ON WHICH THE DIRF BIT WAS
 SET.
STANDARD
 THE VTOC FOR THE VOLUME COULD NOT BE CONVERTED TO THE
 FORMAT FOR ONE OF THE FOLLOWING REASONS:- TWO DATASETS WERE ALLOCATED TO THE SAME SPACE ON THE
 VOLUME.
 AS
 REQUESTS.- A SPLIT CYLINDER DATASET WAS LOCATED ON THE SAME CYLINDER
 A NON-SPLIT CYLINDER DATASET.- THE VTOC CONVERSION ROUTINE IS SET TO REJECT DIRF
...
BECAUSE
 SC13 -      
THE***
 THE ERROR OCCURRED DURING EXECUTION OF A GETMAIN OR FREEMAIN
 MACRO:- A SUBPOOL NUMBER GREATER THAN 127 WAS SPECIFIED BY A
 PROGRAM (A PROGRAM NOT AUTHORIZED TO USE VALID SUBPOOL
 NUMBERS GREATER THAN 127) (RC=08)- AN AUTHORIZED PROGRAM REQUESTED AN INVALID SUBPOOL (RC=04)--POSSIBLE CAUSE--
 TRIED TO CHANGE BLOCKING FACTOR WHILE LOADING A DATASET
 COBOL - CONTROL PASSED BEYOND END OF PROGRAM DUE TO INVALID
 A DATA SET COULD NOT BE CLOSED BY THE CONTROL PROGRAM,
 THE DCB HAD BEEN ERRONEOUSLY MODIFIED.
 ISAM FILE - BAD BLOCK SIZE
 THE ERROR OCCURRED DURING THE EXECUTION OF AN OPEN MACRO FOR
 A CONCATENATED PARTITIONED OR GRAPHICS DATASET.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SC13 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SC13-RC)**--POSSIBLE CAUSE--
 ATTEMPED TO WRITE OUTPUT TO CONCATENATED PARTITIONED
 DATASETS
 ...
 A CONCATENATED PARTITIONED DATASET.  OUTPUT DATASETS CANNOT
 BE
 CONCATENATED.
 SC13 - 18 - AN OPEN MACRO WAS ISSUED FOR A PARTITIONED DATA SET THAT
 WAS CONCATENATED WITH A DATA SET THAT DOES NOT RESIDE ON
 DASD.
 SC37 -      
THE***
 THE ERROR OCCURRED WHEN AN INPUT OPERATION WAS REQUESTED ON
 A DIRECT ACCESS DEVICE.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SC37 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SC37-RC)**
 ...
 CONTAINED A SPLIT CYLINDER ALLOCATION NOT SUPPORTED UNDER
 MVS.
...
- THERE IS NO REAL STORAGE AVAILABLE FOR THE LSQA AQAT TABLE
 ...
 SD05 -      
THE***
 ERROR DURING FREEMAIN.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SD05 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SD05-RC)**
 ...
 SD0A -      
THE***
 ERROR DURING R-FORM GETMAIN OR FREEMAIN.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SD0A OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SD0A-RC)**
 ...
 SD0D -      
SUBTASK
 INVALID ABEND RECURSION DURING ABNORMAL TERMINATION OF
...
MACRO
 ...
 BUFFER.
 SD2D -      
THE TASK WHICH ISSUED A CLOSE FOR A GRAPHIC DEVICE WAS NOT
 THE SAME TASK WHICH PERFORMED THE OPEN.
 THE ERROR OCCURRED DURING THE EXECUTION OF A WTO OR WTOR
 FOR ONE OF THE FOLLOWING REASONS:- THE PARAMETER LIST SUPPLIED TO THE WTOR MACRO DOES NOT
 A FULLWORD BOUNDARY.- A WTOR/MLWTO PARAMATER LIST WAS SPECIFIED.- A MULTILINE WTO WAS SPECIFIED AND SPACE WAS NOT AVAILABLE
 SUBPOOL 229 FOR A WORKAREA FOR SVC 35.- THE PARAMETER LIST PASSED BY THE USER DOES NOT RESIDE IN
 THAT IS ACCESSIBLE BY THE USER.- SPACE WAS NOT AVAILABLE IN SUBPOOL 231 FOR AN ORE OR WQE
 OVERLAY SUPERVISOR FOUND INVALID RECORD TYPE
 BAD LOAD MODULE RE-LINKEDIT
 SD37 - 04 - (04 IS THE ONLY POSSIBLE RETURN CODE ASSOCIATED WITH SD37)
 A DATASET OPENED FOR OUTPUT USED ALL THE PRIMARY SPACE, AND
 NO
 SECONDARY SPACE WAS REQUESTED.  EITHER SPECIFY A LARGER
 PRIMARY
 SD78 -      
THE***
 QUANTITY OR ADD A SECONDARY QUANTITY TO THE REQUEST.
 ERROR DURING RU- OR VRU-FORM GETMAIN OR RU-FORM FREEMAIN.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SD78 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SD78-RC)**
 ...
 OR TO BACK THE RSM CONTROL BLOCKS.
 SE00 -      
AN ERROR OCCURRED DURING EXCP PROCESSING OF AN I/O REQUEST.
X'45'.
 THE IOSB RETURNED TO EXCP TERMINATION HAD A RETURN CODE OF
 THE I/O REQUEST WAS TERMINATED.  NO EXCP DEBUGGING ERROR IS
 SUPPLIED.  A RECORD IS WRITTEN TO SYS1.LOGREC. THE PREVIOUS
 LOGREC ERROR RECORD SHOULD INDICATE THE CAUSE OF THE ABEND.--POSSIBLE CAUSE--
 A PROGRAM CHECK OR MACHINE CHECK OCCURRED WHILE THE IOS IRB
 ANALYSIS MODULES OR MODULE IECVPST WAS EXECUTING.
 A PROGRAM CHECK OR MACHINE CHECK OCCURRED WHILE A
 NONRESIDENT
 PROCESSING
 SE03 -      
ENSURE THAT
 ...
 THE OPERATOR CAUSED A SYSTEM RESTART DURING I/O REQUEST
 A TASK ATTEMPTED TO TERMINATE NORMALLY BUT WAS OPERATING IN
 MUST-COMPLETE STATUS AND DID NOT RESET MUST-COMPLETE.
 EACH ISSUANCE OF AN ENQ, RESERVE AND STATUS MACRO IS PAIRED
 A DEQ OR STATUS MACRO REQUESTING 'RESET MUST-COMPLETE'.
 SE13 - 04 - (04 IS THE ONLY POSSIBLE RETURN CODE ASSOCIATED WITH SE13)
 WHEN OPENING A GRAPHIC DATA CONTROL BLOCK THE GNCP (MAXIMUM
 NUMBER
 OF CONCURRENT CHANNEL PROGRAMS) BYTE CONTAINED A VALUE
 OUTSIDE
 SE1A -      
BECAUSE
 BEEN
...
THE***
 ...
 CAS (THE CATALOG ADDRESS SPACE) ALLOCATE TASK ABENDED
 IT EXCEEDED THE ALLOTTED TIME FOR PROCESSING. IT MAY HAVE
 WAITING FOR SOME PERMANENTLY UNAVAILABLE SYSTEM RESOURCE.
 THE SYSTEM ATTACHES A NEW TASK TO RESTORE THE ALLOCATE
 THE ERROR OCCURRED DURING END OF VOLUME PROCESSING WHEN AN
 OUTPUT OPERATION WAS REQUESTED FOR A DATASET.
 ***IF THE REGISTER 15 RETURN CODE IS AVAILABLE, SUPPLY IT IN
 ***SE37 OPERAND FOR MORE SPECIFIC INFORMATION: H ABEND
 O(SE37-RC)**--POSSIBLE CAUSE--
 ...
 THE CURRENT VOLUME, AND NO MORE VOLUMES WERE AVAILABLE:- NOT ENOUGH VOLUMES WERE SPECIFIED FOR THE DATASET THROUGH
 THE
 'SER', VOLUME COUNT, OR 'REF' SUBPARAMETER OF THE VOLUME
 PARAMETER.  WHEN ALL THE VOLUMES WERE FILLED, THE PROGRAM
 ATTEMPTED TO WRITE ANOTHER RECORD.- FOR A PARTITIONED DATASET ON A DIRECT ACCESS VOLUME, ALL
 SPACE
 RECORD.
 EXTENTS
 WAS FILLED WHEN THE PROGRAM ATTEMPTED TO WRITE ANOTHER
 (A PARTITIONED DATASET CAN RESIDE ON ONLY ONE VOLUME.)- FOR A PARTITIONED DATASET ON A DIRECT ACCESS VOLUME, 16
 ...
 A DSCB WITH A DUPLICATE DATASET NAME ON THE NEXT VOLUME,
 WITH A
 VOLUME SEQUENCE NUMBER LESS THAN THAT IN THE DEB.
WRITTEN- A MULTI-VOLUME PHYSICAL SEQUENTIAL DATASET WAS BEING
 ON A DIRECT ACCESS DEVICE.  ALL SPACE WAS FILLED ON A
 VOLUME,
 SPECIFIED
 OR
 AND AN ATTEMPT WAS MADE TO OBTAIN SPACE ON THE NEXT
 VOLUME.  EITHER THE SPACE WAS NOT AVAILABLE ON THAT VOLUME
...
U4 -        
SET.
 DISABLED CODE WAS EXECUTING AND ISSUED RACHECK (SVC 130)
 IS INACTIVE BECAUSE RACF IS NOT INSTALLED ON THE SYSTEM.
 DISABLED CODE WAS EXECUTING AND ISSUED RACINIT (SVC 131)
 IS INACTIVE BECAUSE RACF IS NOT INSTALLED ON THE SYSTEM.
 DISABLED CODE WAS EXECUTING AND ISSUED RACLIST (SVC 132)
 IS INACTIVE BECAUSE RACF IS NOT INSTALLED ON THE SYSTEM.
 DISABLED CODE WAS EXECUTING AND ISSUED RACDEF (SVC 133)
 IS INACTIVE BECAUSE RACF IS NOT INSTALLED ON THE SYSTEM.
 OVERLAY SUPERVISOR - INVALID SUPERVISOR CALL
 BAD LOAD MODULE. RE-LINKEDIT ENTIRE MODULE.
 HARDWARE ERROR
 GO STEP OF FORTRAN - SUBROUTINE FILENO- NON SEQUENTIAL DATA
 U8   -      GO STEP OF FORTRAN - SUBROUTINE FILENO- IDSRN OUT OF RANGE.
 U12  -      
GO STEP OF FORTRAN - SUBROUTINE FILENO- DATA SET REFERENCE
 NUMBER
 U16  -      
OUTPUT
 'IN' OR
...
HAS NOT BEEN USED.
 ...
 GO STEP OF FORTRAN - MISSING SD4060 DD CARD FOR SD4060
 GO STEP OF FORTRAN - SUBROUTINE FILENO- SECOND ARGUMENT NOT
 'OUT' .
 GO STEP OF FORTRAN - SUBROUTINE FILENO- PREVIOUS FILE NOT
 ASMF COULD NOT OPEN SYSIN - CHECK YOUR SYSIN DD CARD
 I/O FAILURE WHEN READING A BAD TRACK
 GO STEP OF FORTRAN - SUBROUTINE FILENO- DEVICE IS NOT A
 ...
 FDR - DISK I/O ERROR
 FDR - TAPE I/O ERROR
 FDR - TAPE END OF FILE BEFORE DATASET OR TRAILER RECORD
